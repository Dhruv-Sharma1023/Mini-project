...
-------------------------
Handles login, logout, registration, profile, password change, and admin user management.
"""
...
def register():
    """Self-registration — open to anyone. New accounts get 'agent' role."""
    if "user_id" in session:
        return redirect(url_for("main.index"))
    ...
    if request.method == "POST":
        ...
        if action == "change_password":
            ...
            if not full_name:
                ...
                with db_session(current_app.config["DB_PATH"]) as conn:
